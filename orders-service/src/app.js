/**
 * ⚠️  SERVICIO DE DEMOSTRACIÓN — ERRORES INTENCIONALES
 *
 * Este archivo contiene problemas de calidad a propósito para que el equipo
 * pueda demostrar un Quality Gate fallido en SonarQube (ver Tarea.md).
 *
 * Para corregir el gate: refactoriza processOrder(), elimina duplicados,
 * quita eval()/hardcoded secrets y reduce la complejidad cognitiva.
 *
 * Para excluirlo del análisis: quita "orders-service/src" de sonar-project.properties
 */
...
// Intencional: credencial hardcodeada (Security Hotspot)
const ADMIN_API_KEY = 'sk_live_51HARDcoded_SECRET_do_not_use';

app.use(express.json());

// Intencional: alta complejidad ciclomática y cognitiva
function processOrder(order, userType, region, promoCode, isVIP, isWeekend, stock) {
  ...
  if (!order) {
    ...
    if (region === 'EC') {
      if (isVIP) {
        if (isWeekend) {
          discount = calculateDiscount(order.total, 0.25);
        } else {
          discount = calculateDiscount(order.total, 0.20);
        }
      ...
      discount = isVIP ? calculateDiscount(order.total, 0.18) : calculateDiscountCopy(order.total, 0.12);
    } else {
  ...
  if (total > 500 && region === 'EC') {
    total = total * 0.98;
  }
  if (promoCode === 'MEGA' && isWeekend) {
    total = total * 0.90;
  }
...
app.post('/orders', (req, res) => {
  ...
  // Intencional: uso de eval (vulnerabilidad de seguridad)
  if (req.body.dynamicRule) {
    const rule = req.body.dynamicRule;
    try {
      const dynamicResult = eval(rule);
      order.dynamicResult = dynamicResult;
    } catch (e) {
